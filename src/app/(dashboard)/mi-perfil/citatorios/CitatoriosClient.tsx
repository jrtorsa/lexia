...
export default function CitatoriosClient({ hasGoogle }: { hasGoogle: boolean }) {
      ...
      reader.onload = async (e) => {
        ...
        if (res.ok) {
          ...
          setForm(prev => ({
            ...
            fecha: prev.fecha, // fecha requires manual input (ISO format conversion)
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (res.ok) {
      ...
      const msg = saved.googleEventId
        ? 'Citatorio guardado y agregado a Google Calendar'
        : 'Citatorio guardado'
      showToast(msg)
  ...
  return (
    <div className="p-8 relative">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-semibold text-[#0C0D10]" style={{ fontFamily: 'var(--font-cormorant)' }}>
            Mis Citatorios
          </h1>
        ...
        <button
          ...
          className="flex items-center gap-2 bg-[#C49A3C] hover:bg-[#E2B865] text-[#0C0D10] text-sm font-semibold px-4 py-2.5 rounded-lg transition-colors"
        >
        ...
        <>
          ...
          <div className="fixed right-0 top-0 h-full w-full max-w-lg bg-white z-50 shadow-2xl overflow-y-auto">
            <div className="p-6 border-b border-[#EAE4D9] flex items-center justify-between">
              ...
              <button onClick={closePanel} className="text-[#0C0D10]/40 hover:text-[#0C0D10] transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
              ...
              <div>
                ...
                {imagePreview ? (
                  <div className="relative">
                    <img src={imagePreview} alt="preview" className="w-full h-48 object-cover rounded-lg border border-[#EAE4D9]" />
                    <button
                      type="button"
                  ...
                  <button
                    ...
                    className="w-full border-2 border-dashed border-[#EAE4D9] hover:border-[#C49A3C]/40 rounded-xl py-8 flex flex-col items-center gap-2 text-[#0C0D10]/40 hover:text-[#0C0D10]/60 transition-colors"
                  >
  ...
  return (
    <div>
      <label className="flex items-center gap-1.5 text-xs font-semibold text-[#0C0D10]/60 mb-1.5 tracking-wide uppercase">
        {icon}
        {label}
        {required && <span className="text-[#C49A3C]">*</span>}
      </label>
      {children}
    </div>
  )
...
function CitatorioCard({
  citatorio, onDelete, deletingId,
}: {
  citatorio: Citatorio
  ...
  return (
    <div className={`flex gap-4 border rounded-xl p-4 group transition-colors ${cardBorder[status]}`}>
      {/* Date badge */}
      <div className={`flex flex-col items-center justify-center w-14 h-14 rounded-xl flex-shrink-0 font-semibold ${statusColors[status]}`}>
        <span className="text-xl leading-none">{day}</span>
        <span className="text-[10px] tracking-wider mt-0.5">{month}</span>
      </div>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-start justify-between gap-3">
          ...
          <button
            ...
            className="opacity-0 group-hover:opacity-100 text-[#0C0D10]/30 hover:text-red-500 transition-all flex-shrink-0 disabled:opacity-30"
          >
        ...
        {citatorio.notas && (
          <p className="text-xs text-[#0C0D10]/40 mt-1.5 line-clamp-1">{citatorio.notas}</p>
        )}
      </div>
    </div>
  )
